      ...
      <div className="mb-16">
        <div className="grid md:grid-cols-3 gap-6 relative">
            ...
            >
              <Card glow={node.color as "emerald" | "blue" | "violet"} className="h-full">
                <div className="flex items-start gap-4 mb-4">
                  <div className={`p-3 rounded-xl bg-${node.color}/20`}>
                    <node.icon className={`w-6 h-6 text-${node.color}`} />
                  </div>
                  <div>
                  ...
                  <ul className="space-y-2">
                    {node.challenges.map((challenge, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm text-slate-light">
                        <span className={`w-1.5 h-1.5 rounded-full bg-${node.color}`} />
                        {challenge}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="hidden md:block absolute top-1/2 left-1/3 w-1/3 h-px">
            <motion.div
              className="h-full bg-gradient-to-r from-emerald/50 via-blue/50 to-blue/50"
              initial={{ scaleX: 0 }}
          ...
          <div className="hidden md:block absolute top-1/2 right-1/3 w-1/3 h-px">
            <motion.div
              className="h-full bg-gradient-to-r from-blue/50 via-blue/50 to-violet/50"
              initial={{ scaleX: 0 }}
              whileInView={{ scaleX: 1 }}
              viewport={{ once: true }}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {transformationGoals.map((goal, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="p-6 rounded-xl bg-slate-dark/50 border border-slate-mid hover:border-cyan/50 transition-all h-full">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-cyan font-mono text-sm">0{index + 1}</span>
                  <ArrowRight className="w-4 h-4 text-slate-muted group-hover:text-cyan transition-colors" />
                </div>
